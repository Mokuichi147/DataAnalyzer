...
- **DuckDB**: .db, .duckdb ファイル（複数テーブル一括読み込み対応）
...
### 対応機能
- **直接読み込み**: DuckDBデータベースファイル（.db、.duckdb）の直接アップロード
- **複数テーブル一括取り込み**: 1つのDuckDBファイルから全テーブルを自動検出・読み込み
- **スキーマ保持**: 元のテーブル構造とデータ型を完全に保持
- **自動監視追加**: 読み込んだテーブルは自動的にリアルタイム監視対象に追加

### 使用方法
1. DuckDBファイル（.db または .duckdb）をドラッグ&ドロップ
2. ファイルがDuckDBファイルとして自動識別される
3. 「すべて処理」または個別処理でテーブルを読み込み
4. 複数のテーブルが一括でアプリケーションに取り込まれる

### 技術詳細
- **ATTACH機能**: DuckDBのATTACH文を使用してファイルを一時的にアタッチ
- **メタデータ取得**: information_schemaからテーブル一覧を取得
- **データコピー**: CREATE TABLE AS SELECT でデータを現在のセッションにコピー
- **自動クリーンアップ**: 処理完了後にアタッチを自動解除
